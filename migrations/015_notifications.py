...
def migrate(migrator, database, fake=False, **kwargs):
    ...
    class Notification(pw.Model):
        ...
        sub = pw.ForeignKeyField(
            backref="notification_set",
            column_name="sid",
            field="sid",
            model=migrator.orm["sub"],
            null=True,
        )
        post = pw.ForeignKeyField(
            backref="notification_set",
            column_name="pid",
            field="pid",
    ...
    if not fake:
        ...
        qry = (
            ...
            .join(SubPostComment, on=SubPostComment.cid == Message.mlink)
            .join(SubPost)
            .where(Message.mtype << (4, 5))
        )
        ...
        for msg in Message.select().where(Message.mtype == 8):
            ...
            if (
                len(parts) == 5
            ):  # We guess if it's a comment or a post based on the permalink -_-
