...
class Notifications(object):
    ...
    def send(self, notification_type, target, sender, sub=None, comment=None, post=None, content=None):
        ...
        """
        Notification.create(type=notification_type, target=target, sender=sender, sub=sub, comment=comment, post=post,
                            content=content)
        ...
        if self.push_service:
            ...
            if notification_type == 'POST_REPLY':
                ...
                message_body = _("%(name)s replied to your comment in the post titled %(title)s", name=sender.name,
                                 title=post.title)
            elif notification_type in ('POST_MENTION', 'COMMENT_MENTION'):
                message_title = _("You were mentioned in %(prefix)s/%(sub)s", prefix=config.site.sub_prefix,
                                  sub=sub.name)
                message_body = _("%(name)s mentioned you in the post titled %(title)s", name=sender.name,
                                 title=post.title)
            elif notification_type == 'POST_DELETE':
                message_title = _("Your post in %(prefix)s/%(sub)s was deleted", prefix=config.site.sub_prefix,
                                  sub=sub.name)
                message_body = _("%(name)s deleted your post titled %(title)s. %(comment)s", name=sender.name,
                                 title=post.title, comment=content)
            elif notification_type == 'SUB_BAN':
                message_title = _("You have been banned from %(prefix)s/%(sub)s", prefix=config.site.sub_prefix,
                                  sub=sub.name)
                message_body = _("%(name)s banned you with reason: %(content)s", name=sender.name, comment=content)
            elif notification_type == 'SUB_UNBAN':
                message_title = _("You have been unbanned from %(prefix)s/%(sub)s", prefix=config.site.sub_prefix,
                                  sub=sub.name)
                message_body = _("%(name)s unbanned you.", name=sender.name)
            elif notification_type in ('MOD_INVITE', 'MOD_INVITE_JANITOR'):
                message_title = _("You have been invited to moderate %(prefix)s/%(sub)s", prefix=config.site.sub_prefix,
                                  sub=sub.name)
